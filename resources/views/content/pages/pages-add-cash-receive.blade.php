...
<h4 class="py-3 mb-4">
  <span class="text-muted fw-light">Planned Cash /</span> Cash Received Amount
</h4>
...
<div class="row">
  <div class="col-md-12">
      ...
      <input type="text" class="form-control mt-3" name="cashreceived" placeholder="Enter cash receive amount AED"/>
            </div>
...
document.getElementById('plannedCashForm').addEventListener('submit', function(event) {
  ...
  // Get the value of the 'cashreceived' field
  const cashreceivedField = document.querySelector('input[name="cashreceived"]');
  const cashreceivedValue = cashreceivedField.value.trim();

  // Validate the 'cashreceived' field
  if (cashreceivedValue === '') {
    const alertBox = document.getElementById('responseAlert');
    const alertMessage = document.getElementById('alertMessage');

    alertBox.className = 'alert alert-danger alert-dismissible fade show';
    alertMessage.textContent = 'Please enter the cash received amount.';
